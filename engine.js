// Skull game engine (server-authoritative). Mirrors src/lib/skull/engine.ts.
...
export function raiseBid(g, playerId, amount) {
  ...
  ng.currentPlayerIdx = nextActiveIdx(ng, ng.players.findIndex((x) => x.id === playerId));
  while (ng.bid.passed.includes(ng.players[ng.currentPlayerIdx].id)) ng.currentPlayerIdx = nextActiveIdx(ng, ng.currentPlayerIdx);
  return maybeStartReveal(ng);
}
...
function autoFlipOwn(g) {
  let ng = g;
  while (ng.phase === "revealing" && ng.reveal && !ng.reveal.hitSkull && ng.reveal.flipped < ng.reveal.target) {
...
export function flipFromPlayer(g, playerId) {
  ...
  if (card === "skull") {
    ng.reveal.hitSkull = true; ng.reveal.skullOwnerId = target.id;
...
function resolveSuccess(g) {
  ...
  if (bidder.points >= POINTS_TO_WIN) {
    ng.phase = "gameOver"; ng.winnerId = bidder.id;
    log(ng, `👑 ${bidder.name} গেম জিতলো!`); return ng;
  }
...
export function loseCard(g, idx) {
  ...
  if (totalLeft === 0) { loser.eliminated = true; log(ng, `☠️ ${loser.name} বাদ পড়লো!`); }
  ng.pendingLoss = null;
  const alive = activePlayers(ng);
  if (alive.length === 1) {
    ng.phase = "gameOver"; ng.winnerId = alive[0].id;
    log(ng, `👑 ${alive[0].name} জয়ী!`); return ng;
  }
...
function resetRound(g) {
  ...
  ng.currentPlayerIdx = startIdx >= 0 && !ng.players[startIdx].eliminated
    ? startIdx : nextActiveIdx(ng, startIdx >= 0 ? startIdx : 0);
  log(ng, `— রাউন্ড ${ng.round} —`);
  return ng;
}

/** Return a sanitized public view of state for viewer playerId. */
